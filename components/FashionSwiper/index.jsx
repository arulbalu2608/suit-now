...
const FashionSwiper = (props) => {
  ...
  const { slidesPerView = 3, sliderData, leftArrowImage = leftArrow, rightArrowImage = rightArrow,nextEl,prevEl ,el} = props
        ...
        {sliderData?.map((item) => {
          return (
            <React.Fragment key={item.id}>
              <swiper-slide> <Image src={item.path} alt={item.id} /></swiper-slide>
            </React.Fragment>
          )
        })}
      </swiper-container>
